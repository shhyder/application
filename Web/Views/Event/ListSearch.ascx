       ...
       { %>
            <% if (grid_No % 2 == 0)
               ...
               grid_No++;
                ...
                querystring = "strLat=" + ViewData["startLatitude"].ToString() + "&strLog=" + ViewData["startLongitude"].ToString() + "&endLat=" + consumer["Latitude"].ToString() + "&endLog=" + consumer["Longitude"].ToString() + "&id=" + consumer["Event_ID"].ToString() + "&dist=12000" + ViewData["queryString"].ToString();
            ...
            <tr  onmouseover="javascript:itemMouseOver(<%= id - 1 %>,this)" onmouseout="javascript:itemMouseOut(<%= id - 1 %>,this)" onclick="javascript:itemMouseClick(<%= id - 1 %>)">
                <td colspan="2" class="<%=contentStyle %>" style="padding: 0.5em  0.5em 0em  0.5em" >
                    ...
                    <span style="display: inline-block;vertical-align:top; padding: 0.5em 1em 0em 0em; "><b><%= consumer["Event"].ToString()%></b></span>  <br />
                    <%= consumer["Address"].ToString()%><br />
                    <%= consumer["Date"].ToString().ToUpper()%><br />
            ...
            <tr style=" background:LightGray;" onmouseover="javascript:itemMouseOver(<%= id - 1 %>,jQuery(this).prev())" onmouseout="javascript:itemMouseOut(<%= id - 1 %>,jQuery(this).prev())" onclick="javascript:itemMouseClick(<%= id - 1 %>)">
                ...
                <td style="text-align: right;padding: 0.5em 0.5em 0em  0.5em"><a href="javascript:getDirection(<%= consumer["Latitude"].ToString() %>,<%= consumer["Longitude"].ToString() %>,<%= id - 1  %>)" >Directions to Jatai Event</a></td>
            </tr>
            <% id++;
                   sr_No++;
                %>
   ...
   <tr>
    ...
    <tr valign="top">
        <td class="header" colspan=4>
            <b style="font-size:medium">Local Distributor</b><br />
            <b style="font-size: small">Displaying <%= sr_No %>-<%= sr_No + 2 %> of <%= ViewData["Count"]%> Distributors found</b><br />
            Each distributor below carries at least one of the products.
            in your search. Click "Jatai Products" for a list of all Jatai.
            Products carried.<br />
            Please contact store to ensure product availability.
            <br />
        </td>
    </tr>
    ...
    <% foreach (var consumer in (ViewData[UIDealerSearch.listDealer.ToString()] as List<DataRow>))
       { %>
            ...
            <tr>
                <td valign="top" class="<%=contentStyle %>" ><%= sr_No.ToString()%>.</td><% sr_No++; %>
                <td class="<%=contentStyle %>" >
                    <%= consumer["Event"].ToString() %><br />
                    <%= consumer["Date"].ToString()%><br />
                    <%= consumer["Address"].ToString()%><br />
                    <a target="_blank"  href="<%=  Url.Content("~/SearchDealer/Direction?" +  querystring)%>">( Direction )</a><br />
                    <%= consumer["Date"].ToString()%><br />
                    <%= Convert.ToInt32( consumer["distance"] ).ToString()  + " miles"%>
                </td>
                <td>
                    <img src='<%=  Url.Content("~/icon/0" + id.ToString() + ".png")%>' alt="" border="0" width="27" height="27">
                </td>
                <% id++; %>
            </tr>
    <% } %>
    ...
    {%>
        <tr>
            <td colspan="3" class="gridAlternateContentStyle" >
